...
class InterfaxSpider(NewsSpider):
    ...
    start_urls = [
        "https://www.interfax.ru/news/{}".format(datetime.datetime.today().strftime("%Y/%m/%d"))]
    config = NewsSpiderConfig(
        ...
        text_path='//article[contains(@itemprop, "articleBody")]/p[not(contains(@itemprop, "author") or contains(@itemprop, "description"))]//text()',
    ...
    def parse_document(self, response):
        for res in super().parse_document(response):
            if 'INTERFAX.RU - ' in res['text'][0]:
                res['text'][0] = re.search(
                    r'INTERFAX\.RU - ([\d\D]+)', res['text'][0]).group(1)
